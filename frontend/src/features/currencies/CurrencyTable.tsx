/* DOMAIN — Currencies list table.
 *
 * Pattern reference: features/sourceTypes/SourceTypeTable.tsx (locked
 * master-data template). Adds: code column, BASE pill in name cell,
 * current-rate column, decimals column. Toggle-active is hidden for
 * the base currency (backend rejects deactivation; UI hides the noise).
 */
...
interface Props {
  ...
  onToggleActive: (row: Currency) => void
  onSetBase: (row: Currency) => void
  onUpdateRate: (row: Currency) => void
...
export function CurrencyTable({
  ...
  const skeletonRows: Currency[] = showSkeletons
    ? Array.from({ length: 5 }, (_, i) => ({ currencyID: `sk-${i}` }) as unknown as Currency)
    : []
  ...
  return (
    <div className="bg-white rounded-md border border-border-color custom-datatable relative">
      ...
      <div className="px-4 py-3 flex items-center justify-between flex-wrap gap-2 custom-table-header">
        <h5 className="text-[17.5px]">Currencies</h5>
          ...
          <Column
            ...
            body={(row: Currency) =>
              showSkeletons ? (
                ...
                <span className="font-mono font-semibold text-dark">{row.code}</span>
              )
            }
          />
          <Column
          ...
          <Column
            ...
            body={(row: Currency) =>
              showSkeletons ? (
                ...
                <span className="text-default text-sm">— (base)</span>
              ) : row.currentRate ? (
                <span className="font-mono text-dark">
                  {trimTrailingZeros(row.currentRate.rate)}
                </span>
              ) : (
                <span className="text-danger text-sm">No rate</span>
              )
            }
          />
          <Column
            ...
            className="px-4 py-3 text-start text-dark"
            body={(row: Currency) =>
              showSkeletons ? <Skel w="20%" /> : String(row.decimalPlaces)
            }
          />
          <Column
            field="isActive"
        ...
        <div className="sm:col-span-4 col-span-12">
          <div className="flex justify-center sm:block">
            <div className="dt-length">
              <label htmlFor="currencies-page-size" className="text-sm">
                Show{' '}
                <select
                  id="currencies-page-size"
                  className="dt-input"
                  value={limit}
...
const SORT_OPTIONS: { label: string; sort: SortField; order: SortDir }[] = [
  { label: 'Display Order',     sort: 'displayOrder', order: 'asc'  },
  { label: 'Base First',        sort: 'isBase',       order: 'desc' },
            ...
            <div className="p-2 space-y-1">
              ...
              {!row.isBase && (
                <button
                  type="button"
                  role="menuitem"
                  onClick={() => { setOpen(false); onUpdateRate(row) }}
                  className="flex items-center cursor-pointer w-full px-4 py-1.75 rounded-lg text-sm text-default hover:bg-light hover:text-primary"
                >
                  <IconCurrencyDollar size={14} className="me-2" />
                  Update Rate
                </button>
              )}
              {!row.isBase && row.isActive && (
                <button
                  type="button"
                  role="menuitem"
                  onClick={() => { setOpen(false); onSetBase(row) }}
                  className="flex items-center cursor-pointer w-full px-4 py-1.75 rounded-lg text-sm text-default hover:bg-light hover:text-primary"
                >
                  <IconStar size={14} className="me-2" />
                  Set as Base
                </button>
              )}
              {!row.isBase && (
...
/** "1.000000" → "1", "0.270000" → "0.27", "0.123456" → "0.123456". */
