...
interface Props {
  ...
  onView: (row: SourceType) => void
  onEdit: (row: SourceType) => void
  onDelete: (row: SourceType) => void
  onToggleActive: (row: SourceType) => void
...
export function SourceTypeTable({
  ...
  const skeletonRows: SourceType[] = showSkeletons
    ? Array.from({ length: 5 }, (_, i) => ({ sourceTypeID: `sk-${i}` }) as unknown as SourceType)
    : []
  ...
  return (
    <div className="bg-white rounded-md border border-border-color custom-datatable relative">
      ...
      <div className="px-4 py-3 flex items-center justify-between flex-wrap gap-2 custom-table-header">
        <h5 className="text-[17.5px]">Source Types</h5>
      ...
      <div className="overflow-x-auto">
        <DataTable
          ...
          dataKey="sourceTypeID"
          emptyMessage={
            <div className="py-10 text-center text-sm text-default">
              No source types match your filters.
            </div>
          }
        >
          <Column
          ...
          <Column
            ...
            className="px-4 py-3 text-start text-dark"
            body={(row: SourceType) =>
              showSkeletons ? <Skel w="50%" /> : row.label
            }
          />
          <Column
            field="createdAt"
        ...
        <div className="sm:col-span-4 col-span-12">
          <div className="flex justify-center sm:block">
            <div className="dt-length">
              <label htmlFor="source-types-page-size" className="text-sm">
                Show{' '}
                <select
                  id="source-types-page-size"
                  className="dt-input"
                  value={limit}
...
const SORT_OPTIONS: { label: string; sort: SortField; order: SortDir }[] = [
  ...
  { label: 'Label (A-Z)',      sort: 'label',     order: 'asc'  },
  { label: 'Label (Z-A)',      sort: 'label',     order: 'desc' },
]
