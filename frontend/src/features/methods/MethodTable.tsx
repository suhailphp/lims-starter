/* DOMAIN — Method list table.
 * Pattern reference: features/tests/TestTable.tsx (single-FK template).
 * Adds: Default badge column, code-based search (no name).
 */
...
interface Props {
  ...
  onView: (row: Method) => void
  onEdit: (row: Method) => void
  onDelete: (row: Method) => void
  onToggleActive: (row: Method) => void
...
export function MethodTable({
  ...
  const skeletonRows: Method[] = showSkeletons
    ? Array.from({ length: 5 }, (_, i) => ({ methodID: `sk-${i}` }) as unknown as Method)
    : []
  ...
  return (
    <div className="bg-white rounded-md border border-border-color custom-datatable relative">
      ...
      <div className="px-4 py-3 flex items-center justify-between flex-wrap gap-2 custom-table-header">
        <h5 className="text-[17.5px]">Methods</h5>
            ...
            body={(row: Method) =>
              showSkeletons ? (
                ...
                <button
                  type="button"
                  onClick={() => onView(row)}
                  className="inline-block text-left cursor-pointer hover:text-primary hover:underline text-dark font-medium font-mono"
                >
                  {row.code}
                </button>
              )
            ...
            body={(row: Method) =>
              showSkeletons ? (
                ...
                <div className="flex flex-col">
                  <span className="text-dark">{row.test.name}</span>
                  {row.test.category?.name && (
                    <span className="text-xs text-default">{row.test.category.name}</span>
                  )}
                </div>
              ) : (
        ...
        <div className="sm:col-span-4 col-span-12">
          <div className="flex justify-center sm:block">
            <div className="dt-length">
              <label htmlFor="methods-page-size" className="text-sm">
                Show{' '}
                <select
                  id="methods-page-size"
                  className="dt-input"
                  value={limit}
