/* DOMAIN — Method delete confirmation. */
import { useState } from 'react'
import axios from 'axios'
import { Dialog } from '@/components/ui/Dialog'
import { useDeleteMethod } from './methodsQueries'
...
export function MethodDeleteDialog({ method, onClose }: Props) {
  const del = useDeleteMethod()
  ...
  const handleConfirm = async () => {
    ...
    setError(null)
    try {
      await del.mutateAsync(method.methodID)
      toast.success(`Method "${method.code}" deleted`)
      onClose()
    } catch (err) {
      const msg = formatErr(err)
      setError(msg)
      toast.error(`Failed to delete method: ${msg}`)
    }
  }
  ...
  return (
    ...
    >
      <p className="text-sm text-default">
        Delete <strong className="text-dark font-mono">{method?.code}</strong>?
        This will remove the method from your active list.
      </p>
      {error && (
