/* DOMAIN — Source delete confirmation. */
import { useState } from 'react'
import axios from 'axios'
import { Dialog } from '@/components/ui/Dialog'
import { useDeleteSource } from './sourcesQueries'
...
export function SourceDeleteDialog({ source, onClose }: Props) {
  ...
  const handleConfirm = async () => {
    ...
    setError(null)
    try {
      await del.mutateAsync(source.sourceID)
      toast.success(`Source "${source.sourceName}" deleted`)
      onClose()
    } catch (err) {
  ...
  return (
    ...
    >
      <p className="text-sm text-default">
        Delete <strong className="text-dark">{source?.sourceName}</strong>? This
        will remove the source from your active list.
      </p>
      {error && (
