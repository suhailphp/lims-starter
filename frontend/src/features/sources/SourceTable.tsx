/* DOMAIN — Source list table.
 *
 * Pattern reference: features/customers/CustomerTable.tsx (locked template).
 * Adds FK columns rendered from nested `customer.name` / `sourceType.name` /
 * `category.name` (included by GET /api/sources).
 */
...
interface Props {
  ...
  onView: (row: Source) => void
  onEdit: (row: Source) => void
  onDelete: (row: Source) => void
  onToggleActive: (row: Source) => void
...
export function SourceTable({
  ...
  const skeletonRows: Source[] = showSkeletons
    ? Array.from({ length: 5 }, (_, i) => ({ sourceID: `sk-${i}` }) as unknown as Source)
    : []
  ...
  return (
    <div className="bg-white rounded-md border border-border-color custom-datatable relative">
      ...
      <div className="px-4 py-3 flex items-center justify-between flex-wrap gap-2 custom-table-header">
        <h5 className="text-[17.5px]">Sources</h5>
          ...
          <Column
            ...
            body={(row: Source) =>
              showSkeletons ? (
                ...
                row.customer?.name ?? <span className="text-gray-400">—</span>
              )
            }
          />
          <Column
          ...
          <Column
            ...
            body={(row: Source) =>
              showSkeletons ? (
                ...
                row.sourceType?.name ?? <span className="text-gray-400">—</span>
              )
            }
          />
          <Column
          ...
          <Column
            ...
            body={(row: Source) =>
              showSkeletons ? (
                ...
                row.equipmentName ?? <span className="text-gray-400">—</span>
              )
            }
          />
          <Column
        ...
        <div className="sm:col-span-4 col-span-12">
          <div className="flex justify-center sm:block">
            <div className="dt-length">
              <label htmlFor="sources-page-size" className="text-sm">
                Show{' '}
                <select
                  id="sources-page-size"
                  className="dt-input"
                  value={limit}
...
const SORT_OPTIONS: { label: string; sort: SortField; order: SortDir }[] = [
  ...
  { label: 'Source (A-Z)',     sort: 'sourceName',    order: 'asc'  },
  { label: 'Source (Z-A)',     sort: 'sourceName',    order: 'desc' },
  { label: 'Equipment (A-Z)',  sort: 'equipmentName', order: 'asc'  },
  { label: 'Equipment (Z-A)',  sort: 'equipmentName', order: 'desc' },
]
