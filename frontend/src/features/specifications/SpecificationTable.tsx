/* DOMAIN — Specification list table.
 * Pattern reference: features/sourceTypes/SourceTypeTable.tsx (locked template).
 * Specification has only `name` (no `label`) so column count is reduced.
 */
...
interface Props {
  ...
  onView: (row: Specification) => void
  onEdit: (row: Specification) => void
  onDelete: (row: Specification) => void
  onToggleActive: (row: Specification) => void
...
export function SpecificationTable({
  ...
  const skeletonRows: Specification[] = showSkeletons
    ? Array.from({ length: 5 }, (_, i) => ({ specificationID: `sk-${i}` }) as unknown as Specification)
    : []
  ...
  return (
    <div className="bg-white rounded-md border border-border-color custom-datatable relative">
      ...
      <div className="px-4 py-3 flex items-center justify-between flex-wrap gap-2 custom-table-header">
        <h5 className="text-[17.5px]">Specifications</h5>
        ...
        <div className="sm:col-span-4 col-span-12">
          <div className="flex justify-center sm:block">
            <div className="dt-length">
              <label htmlFor="specifications-page-size" className="text-sm">
                Show{' '}
                <select
                  id="specifications-page-size"
                  className="dt-input"
                  value={limit}
