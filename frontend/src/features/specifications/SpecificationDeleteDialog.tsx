/* DOMAIN — Specification delete confirmation. */
import { useState } from 'react'
import axios from 'axios'
import { Dialog } from '@/components/ui/Dialog'
import { useDeleteSpecification } from './specificationsQueries'
...
export function SpecificationDeleteDialog({ specification, onClose }: Props) {
  const del = useDeleteSpecification()
  ...
  const handleConfirm = async () => {
    ...
    setError(null)
    try {
      await del.mutateAsync(specification.specificationID)
      toast.success(`Specification "${specification.name}" deleted`)
      onClose()
    } catch (err) {
      const msg = formatErr(err)
      setError(msg)
      toast.error(`Failed to delete specification: ${msg}`)
    }
  }
  ...
  return (
    ...
    >
      <p className="text-sm text-default">
        Delete <strong className="text-dark">{specification?.name}</strong>? This
        will remove the specification from your active list.
      </p>
      {error && (
