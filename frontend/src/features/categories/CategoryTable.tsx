/* DOMAIN — Category list table.
 *
 * Pattern reference: features/sourceTypes/SourceTypeTable.tsx (locked template).
 */
...
interface Props {
  ...
  onToggleActive: (row: Category) => void
...
export function CategoryTable({
  ...
  const skeletonRows: Category[] = showSkeletons
    ? Array.from({ length: 5 }, (_, i) => ({ categoryID: `sk-${i}` }) as unknown as Category)
    : []
  ...
  return (
    <div className="bg-white rounded-md border border-border-color custom-datatable relative">
      ...
      <div className="px-4 py-3 flex items-center justify-between flex-wrap gap-2 custom-table-header">
        <h5 className="text-[17.5px]">Categories</h5>
        ...
        <div className="sm:col-span-4 col-span-12">
          <div className="flex justify-center sm:block">
            <div className="dt-length">
              <label htmlFor="categories-page-size" className="text-sm">
                Show{' '}
                <select
...
/* Type badge — same shape as status pill, color-coded by enum. */
export function TypeBadge({ type }: { type: 'FUEL' | 'LUBRICANT' | 'WATER' }) {
  const cls =
    type === 'FUEL'
      ? 'bg-warning-50 text-warning-700 border-warning'
      : type === 'LUBRICANT'
        ? 'bg-info-50 text-info-700 border-info'
        : 'bg-primary-50 text-primary border-primary'
  return (
    <span
      className={`inline-flex items-center badge rounded-lg text-xs font-medium border ${cls}`}
    >
      {type}
    </span>
  )
...
const SORT_OPTIONS: { label: string; sort: SortField; order: SortDir }[] = [
  ...
  { label: 'Type (A-Z)',       sort: 'type',      order: 'asc'  },
  { label: 'Type (Z-A)',       sort: 'type',      order: 'desc' },
]
