/* DOMAIN — Unit delete confirmation. */
import { useState } from 'react'
import axios from 'axios'
import { Dialog } from '@/components/ui/Dialog'
import { useDeleteUnit } from './unitsQueries'
...
export function UnitDeleteDialog({ unit, onClose }: Props) {
  ...
  const handleConfirm = async () => {
    ...
    setError(null)
    try {
      await del.mutateAsync(unit.unitID)
      toast.success(`Unit "${unit.name} (${unit.symbol})" deleted`)
      onClose()
    } catch (err) {
      const msg = formatErr(err)
      setError(msg)
      toast.error(`Failed to delete unit: ${msg}`)
    }
  }
  ...
  return (
    ...
    >
      <p className="text-sm text-default">
        ...
        ? This will remove the unit from your active list.
      </p>
      {error && (
