/* DOMAIN — Tax Rates list table.
 *
 * Pattern reference: features/currencies/CurrencyTable.tsx (master-data
 * template + default-flag pattern). Default badge mirrors Currency's
 * Base badge. Toggle-active is hidden for the default tax rate
 * (backend rejects deactivation; UI hides the noise).
 */
...
interface Props {
  ...
  onToggleActive: (row: TaxRate) => void
...
export function TaxRateTable({
  ...
  const skeletonRows: TaxRate[] = showSkeletons
    ? Array.from({ length: 5 }, (_, i) => ({ taxRateID: `sk-${i}` }) as unknown as TaxRate)
    : []
  ...
  return (
    <div className="bg-white rounded-md border border-border-color custom-datatable relative">
      ...
      <div className="px-4 py-3 flex items-center justify-between flex-wrap gap-2 custom-table-header">
        <h5 className="text-[17.5px]">Tax Rates</h5>
          ...
          <Column
            ...
            body={(row: TaxRate) =>
              showSkeletons ? <Skel w="30%" /> : (
                <span className="font-mono">{trimTrailingZeros(row.rate)}%</span>
              )
            }
          />
          <Column
            ...
            className="px-4 py-3 text-start text-dark"
            body={(row: TaxRate) =>
              showSkeletons ? <Skel w="40%" /> : (
                <span className="text-sm text-default">{row.type}</span>
              )
            }
          />
          <Column
            ...
            className="px-4 py-3 text-start text-dark"
            body={(row: TaxRate) =>
              showSkeletons ? <Skel w="20%" /> : String(row.displayOrder)
            }
          />
          <Column
            field="isActive"
        ...
        <div className="sm:col-span-4 col-span-12">
          <div className="flex justify-center sm:block">
            <div className="dt-length">
              <label htmlFor="taxrates-page-size" className="text-sm">
                Show{' '}
                <select
                  id="taxrates-page-size"
                  className="dt-input"
                  value={limit}
...
const SORT_OPTIONS: { label: string; sort: SortField; order: SortDir }[] = [
  ...
  { label: 'Default First',  sort: 'isDefault',    order: 'desc' },
  { label: 'Code (A-Z)',     sort: 'code',         order: 'asc'  },
  { label: 'Code (Z-A)',     sort: 'code',         order: 'desc' },
  { label: 'Name (A-Z)',     sort: 'name',         order: 'asc'  },
  { label: 'Name (Z-A)',     sort: 'name',         order: 'desc' },
  { label: 'Rate (Low-High)',sort: 'rate',         order: 'asc'  },
  { label: 'Rate (High-Low)',sort: 'rate',         order: 'desc' },
            ...
            <div className="p-2 space-y-1">
              ...
              {!row.isDefault && row.isActive && (
                <button
                  type="button"
                  role="menuitem"
                  onClick={() => { setOpen(false); onSetDefault(row) }}
                  className="flex items-center cursor-pointer w-full px-4 py-1.75 rounded-lg text-sm text-default hover:bg-light hover:text-primary"
                >
