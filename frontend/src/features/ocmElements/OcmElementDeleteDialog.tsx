/* DOMAIN — OcmElement delete confirmation. */
import { useState } from 'react'
import axios from 'axios'
import { Dialog } from '@/components/ui/Dialog'
import { useDeleteOcmElement } from './ocmElementsQueries'
...
export function OcmElementDeleteDialog({ ocmElement, onClose }: Props) {
  const del = useDeleteOcmElement()
  const [error, setError] = useState<string | null>(null)

  const handleConfirm = async () => {
    if (!ocmElement) return
    setError(null)
    try {
      await del.mutateAsync(ocmElement.ocmElementID)
      toast.success(`Element "${ocmElement.name} (${ocmElement.symbol})" deleted`)
      onClose()
    } catch (err) {
      const msg = formatErr(err)
      setError(msg)
      toast.error(`Failed to delete element: ${msg}`)
    }
  }
    ...
    >
      <p className="text-sm text-default">
        ...
        <strong className="text-dark">
          {ocmElement?.name} ({ocmElement?.symbol})
        </strong>
        ? This will remove the element from your active list.
      </p>
      {error && (
