/* DOMAIN — OcmElement list table.
 * Pattern reference: features/categories/CategoryTable.tsx (locked template).
 * Compact range display: "min – max unit" (or "—" when both unset).
 */
...
interface Props {
  ...
  onView: (row: OcmElement) => void
  onEdit: (row: OcmElement) => void
  onDelete: (row: OcmElement) => void
  onToggleActive: (row: OcmElement) => void
  onPageChange: (page: number, limit: number) => void
  onSortChange: (sort: SortField, order: SortDir) => void
  filterMenu?: ReactNode
}

export function OcmElementTable({
  rows,
  total,
  isLoading,
  isFetching,
  ...
  const skeletonRows: OcmElement[] = showSkeletons
    ? Array.from({ length: 5 }, (_, i) => ({ ocmElementID: `sk-${i}` }) as unknown as OcmElement)
    : []
  ...
  return (
    <div className="bg-white rounded-md border border-border-color custom-datatable relative">
      ...
      <div className="px-4 py-3 flex items-center justify-between flex-wrap gap-2 custom-table-header">
        <h5 className="text-[17.5px]">OCM Elements</h5>
        <div className="flex flex-wrap items-center gap-2">
          <div className="dt-search">
            <input
              ...
              aria-label="Search OCM elements"
            />
          </div>
          {filterMenu}
      ...
      <div className="overflow-x-auto">
        ...
        >
            ...
            body={(row: OcmElement) =>
              showSkeletons ? (
                ...
                <button
                  type="button"
                  onClick={() => onView(row)}
                  className="inline-block text-left cursor-pointer hover:text-primary hover:underline text-dark font-mono font-semibold"
                >
                  {row.symbol}
                </button>
              )
          ...
          <Column
            ...
            className="px-4 py-3 text-start text-dark"
            body={(row: OcmElement) =>
              showSkeletons ? <Skel w="40%" /> : row.unit
            }
          />
          <Column
            header="Normal"
            ...
            body={(row: OcmElement) =>
              showSkeletons ? (
                ...
                <RangeText min={row.normalRangeMin} max={row.normalRangeMax} />
              )
            }
          />
          <Column
            ...
            body={(row: OcmElement) =>
              showSkeletons ? (
                ...
                <RangeText min={row.cautionRangeMin} max={row.cautionRangeMax} />
              )
            }
          />
          <Column
            ...
            body={(row: OcmElement) =>
              showSkeletons ? (
                ...
                <RangeText min={row.criticalRangeMin} max={row.criticalRangeMax} />
              )
            }
          />
          <Column
        ...
        <div className="sm:col-span-4 col-span-12">
          <div className="flex justify-center sm:block">
            <div className="dt-length">
              <label htmlFor="ocm-elements-page-size" className="text-sm">
                Show{' '}
                <select
                  id="ocm-elements-page-size"
                  className="dt-input"
                  value={limit}
...
/* Compact range cell — "min – max" or "—" when both unset. */
function RangeText({
  min,
  max,
}: {
  ...
  if (a === '') return <span>≤ {b}</span>
  if (b === '') return <span>≥ {a}</span>
  return (
    <span>
      {a} – {b}
    </span>
...
function formatNum(v: number | string | null | undefined): string {
  ...
  // Strip trailing zeros after decimal: "100.0000" → "100", "0.5000" → "0.5"
...
const SORT_OPTIONS: { label: string; sort: SortField; order: SortDir }[] = [
  ...
  { label: 'Symbol (A-Z)',     sort: 'symbol',    order: 'asc'  },
  { label: 'Symbol (Z-A)',     sort: 'symbol',    order: 'desc' },
