/* DOMAIN — Equipment delete confirmation. */
import { useState } from 'react'
import axios from 'axios'
import { Dialog } from '@/components/ui/Dialog'
import { useDeleteEquipment } from './equipmentsQueries'
...
export function EquipmentDeleteDialog({ equipment, onClose }: Props) {
  ...
  const handleConfirm = async () => {
    ...
    setError(null)
    try {
      await del.mutateAsync(equipment.equipmentID)
      toast.success(`Equipment "${equipment.name}" deleted`)
      onClose()
    } catch (err) {
      const msg = formatErr(err)
      setError(msg)
      toast.error(`Failed to delete equipment: ${msg}`)
    }
  }
  ...
  return (
    ...
    >
      <p className="text-sm text-default">
        Delete <strong className="text-dark">{equipment?.name}</strong>? This
        will remove the equipment from your active list.
      </p>
      {error && (
