...
interface Props {
  ...
  onView: (row: Equipment) => void
  onEdit: (row: Equipment) => void
  onDelete: (row: Equipment) => void
  onToggleActive: (row: Equipment) => void
...
export function EquipmentTable({
  ...
  const skeletonRows: Equipment[] = showSkeletons
    ? Array.from({ length: 5 }, (_, i) => ({ equipmentID: `sk-${i}` }) as unknown as Equipment)
    : []
  ...
  return (
    <div className="bg-white rounded-md border border-border-color custom-datatable relative">
      ...
      <div className="px-4 py-3 flex items-center justify-between flex-wrap gap-2 custom-table-header">
        <h5 className="text-[17.5px]">Equipment</h5>
          ...
          <Column
            ...
            body={(row: Equipment) =>
              showSkeletons ? (
                ...
                row.serialNumber ?? <span className="text-gray-400">—</span>
              )
            }
          />
          <Column
        ...
        <div className="sm:col-span-4 col-span-12">
          <div className="flex justify-center sm:block">
            <div className="dt-length">
              <label htmlFor="equipments-page-size" className="text-sm">
                Show{' '}
                <select
...
const SORT_OPTIONS: { label: string; sort: SortField; order: SortDir }[] = [
  ...
  { label: 'Serial (A-Z)',           sort: 'serialNumber',       order: 'asc'  },
  { label: 'Calibration Due (soon)', sort: 'calibrationDueDate', order: 'asc'  },
  { label: 'Calibration Due (far)',  sort: 'calibrationDueDate', order: 'desc' },
]
