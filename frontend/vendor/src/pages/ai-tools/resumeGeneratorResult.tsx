...
const ResumeGeneratorResult = () => {
  ...
  useEffect(() => {
    // Show loading state for 1 second, then show content
...
useEffect(() => {
  ...
  return (
    <>
      ...
      <div className="page-wrapper relative">
        <div className="bg-border-color w-full h-1.5 rounded-lg loading-state absolute top-0 start-0">
          <div className="rounded-lg h-1.5 bg-cyan" style={{ width: "70%" }} />
        </div>
    {/* Flyer Topbar */}
    ...
    {/* End Flyer Topbar */}
    <div className="relative flex flex-col flex-wrap items-center justify-center h-[calc(100vh-120px)]! max-md:h-screen! w-full z-1">
      <div className="p-6">
        {/* Start Grid  */}
        <div className="w-full">
          ...
          <div className={`flex flex-col items-center justify-center loading-state ${!isLoading ? 'hidden' : ''}`}>
            <div className="flex items-center bg-white border border-primary mb-6 p-2 rounded-full ps-5 max-w-80 w-full">
              ...
              />
              <button className="w-9 h-9 rounded-full flex items-center justify-center bg-[image:var(--background-image-linear-gradient-300)] text-white">
                <i className="icon-circle-pause" />
              </button>
            </div>
            <h6 className="mb-2"> Generating Resume... </h6>
            <p className="mb-0">
              {" "}
          ...
          <div className={`content-state ${isLoading ? 'hidden' : ''}`}>
            ...
            {/* Start grid */}
            <div className="flex flex-row flex-wrap md:flex-col items-center gap-2 relative md:fixed md:start-3 start-0 inset-y-0 my-auto h-auto md:h-screen justify-center mb-4 md:mb-0">
              <Link
                to="#"
            ...
            {/* End Grid */}
            {/* Start Resume */}
            <div className="flex items-center justify-center w-full !max-w-[455px] custom-resumetemplate">
              <div id="tab1" className={`${tabhide === 0 ? "block" : "hidden"}`} onClick={()=>settabhide(0)}>
                <Link to="#"
                  onClick={() => handleLightbox(Images.resume_3)}
                  className="image-popup h-full rounded-lg shadow cursor-pointer w-full"
                >
                  <ImageWithBasePath
                    src={Images.resume_3}
                    alt="resume image"
              ...
              <div className={tabhide === 1 ? "block" : "hidden"} id="tab2" onClick={()=>settabhide(1)}>
                <button
                  onClick={() => handleLightbox(Images.resume_2)}
                  className="image-popup h-full rounded-lg shadow cursor-pointer w-full"
                >
                  <ImageWithBasePath
                    src={Images.resume_2}
              ...
              <div className={tabhide === 2 ? "block" : "hidden"} id="tab3" onClick={()=>settabhide(2)}>
                <button
                  onClick={() => handleLightbox(Images.resume_1)}
                  className="image-popup h-full rounded-lg shadow cursor-pointer w-full"
                >
                  <ImageWithBasePath
                    src={Images.resume_1}
              ...
              <div className={tabhide === 3 ? "block" : "hidden"} id="tab4" onClick={()=>settabhide(3)}>
                <button
                  onClick={() => handleLightbox(Images.resume_4)}
                  className="image-popup h-full rounded-lg shadow cursor-pointer w-full"
                >
                  <ImageWithBasePath
                    src={Images.resume_4}
              ...
              <div className={tabhide === 4 ? "block" : "hidden"} id="tab5" onClick={()=>settabhide(4)}>
                <button
                  onClick={() => handleLightbox(Images.resume_5)}
                  className="image-popup h-full rounded-lg shadow cursor-pointer w-full"
                >
                  <ImageWithBasePath
                    src={Images.resume_5}
              ...
              <div className={tabhide === 5 ? "block" : "hidden"} id="tab6" onClick={()=>settabhide(5)}>
                <button
                  onClick={() => handleLightbox(Images.resume_6)}
                  className="image-popup h-full rounded-lg shadow cursor-pointer w-full"
                >
                  <ImageWithBasePath
                    src={Images.resume_6}
  ...
  <div
    id="generate-resume"
    className="remove-overlay hs-overlay hs-overlay-open:translate-x-0 hidden translate-x-full fixed bottom-0 end-0 transition-all duration-300 overflow-hidden transform md:max-w-[400px] max-w-[300px] rounded-lg m-2 md:m-6 border border-border-color shadows w-full z-80 bg-white"
    role="dialog"
    tabIndex={-1}
  ...
  <div
    id="generate-color"
    className="hs-overlay hs-overlay-open:translate-x-0 hidden translate-x-full fixed bottom-0 end-0 transition-all duration-300 transform md:max-w-[400px] max-w-[300px] rounded-lg m-2 md:m-6 border border-border-color shadows w-full z-80 bg-white"
    role="dialog"
    tabIndex={-1}
  ...
  <Lightbox
    ...
    open={lightbox}
    index={selectedImage ? [Images.resume_1, Images.resume_2, Images.resume_3, Images.resume_4, Images.resume_5, Images.resume_6].indexOf(selectedImage) : 0}
...
export default ResumeGeneratorResult
